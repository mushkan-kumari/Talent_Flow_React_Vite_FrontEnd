...
export const JobsBoard = () => {
  ...
  // Helper to update filteredJobs and reset page
  const updateFilteredJobs = (jobsList = jobs) => {
  ...
  // Update filteredJobs when filters or search change
  useEffect(() => {
    updateFilteredJobs();
  ...
  const saveJob = async () => {
    ...
    if (editingJob) {
      const res = await fetch(`/jobs/${editingJob.id}`, { method: "PATCH", body: JSON.stringify(body) });
      const updatedJob = await res.json();
      const newJobsList = jobs.map(j => (j.id === updatedJob.id ? updatedJob : j));
      setJobs(newJobsList);
      updateFilteredJobs(newJobsList);
    } else {
      const res = await fetch("/jobs", { method: "POST", body: JSON.stringify({ 
      ...body, 
      status: "active" // ensure newly created job is set as active
  }) });
      const newJob = await res.json();

      newJob.order = jobs.length;
      const newJobsList = [...jobs, newJob];
      setJobs(newJobsList);
      updateFilteredJobs(newJobsList);
    }
    closeModal();
  ...
  const toggleStatus = async (job) => {
    ...
    const res = await fetch(`/jobs/${job.id}`, { method: "PATCH", body: JSON.stringify({ status: updatedJob.status }) });
    const data = await res.json();
    const newJobsList = jobs.map(j => (j.id === data.id ? data : j));
    setJobs(newJobsList);
    updateFilteredJobs(newJobsList);
  };
  ...
  const deleteJob = async (job) => {
    if (window.confirm(`Delete job "${job.title}"?`)) {
      ...
      const newJobsList = jobs.filter(j => j.id !== job.id);
      setJobs(newJobsList);
      updateFilteredJobs(newJobsList);
    }
  };
  ...
  const handleDragEnd = async (result) => {
    ...
    setJobs(items);
    updateFilteredJobs(items);

    // Update job order in backend (Mirage JS)
    for (let j of items) {
