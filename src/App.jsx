...
import { ensureSeed } from "./db/indexedDB";
import logo from "./assets/react.svg"; // Replace with your logo path

const linkClass = ({ isActive }) =>
  `px-4 py-2 rounded-lg transition-colors duration-200 hover:bg-blue-100 ${
    isActive ? "bg-blue-200 font-semibold" : "text-blue-600"
  }`;
...
export default function App() {
  ...
  if (error) {
    return (
      <div className="flex h-screen items-center justify-center bg-red-50 text-red-700 text-lg">
        ⚠️ Error initializing database: {error}
      </div>
    );
  }
  ...
  return (
    <Router>
      ...
      <nav className="flex items-center justify-between p-4 bg-black shadow-md sticky top-0 z-10">

        <div className="flex items-center gap-3">
          <img src={logo} alt="Portal Logo" className="w-10 h-10 rounded-full" />
      ...
      <div className="p-6 min-h-screen bg-gray-50 flex justify-center items-start">
        <Routes>
          ...
          <Route path="/assessments" element={<AssessmentBuilder jobId="job-1" />} />
